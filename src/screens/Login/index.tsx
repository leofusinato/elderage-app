...
export default function Login() {
  ...
  const handleLogin = useCallback(async () => {
    if (email.length > 0 && password.length > 0) {
      const response = await api.post<AuthLoginResponse>('/login', { email, password });
      console.log(response.status);
      if (response.status === 200) {
        const { user: { id, name, email }, token } = response.data;
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.main}>
        ...
        <AuthInput placeholder='E-mail' leftIcon='user' keyboardType='email-address' value={email} onChangeText={(text) => setEmail(text)} />
        <AuthInput placeholder='Digite sua senha' leftIcon='lock' isPassword value={password} onChangeText={(text) => setPassword(text)} />
